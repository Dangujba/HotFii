...
@if($allowance && $allowance['remaining_seconds'] !== null)<div class="col-6"><div class="p-3 rounded bg-body-tertiary"><div class="small text-secondary">Time remaining</div><strong>{{ \Carbon\CarbonInterval::seconds((int) $allowance['remaining_seconds'])->cascade()->forHumans(['short' => true, 'parts' => 3]) }}</strong></div></div>@endif
@if($allowance && $allowance['remaining_bytes'] !== null)<div class="col-6"><div class="p-3 rounded bg-body-tertiary"><div class="small text-secondary">Data remaining</div><strong>{{ number_format($allowance['remaining_bytes'] / 1048576, 1) }} MB</strong></div></div>@endif<div class="col-6"><div class="p-3 rounded bg-body-tertiary"><div class="small text-secondary">Valid until</div><strong>{{ $voucher->expires_at?->copy()->setTimezone($device->organization->timezone ?: config('app.timezone'))->format('d M Y, H:i T') ?? 'Plan allowance used' }}</strong></div></div><div class="col-6"><div class="p-3 rounded bg-body-tertiary"><div class="small text-secondary">Device limit</div><strong>{{ $voucher->batch->accessPlan->simultaneous_use }}</strong></div></div></div>
...
]) }}"><button class="btn btn-hotfii btn-lg w-100">Connect to Internet</button></form>
@else<div class="alert alert-info text-start"><strong>Laboratory credentials</strong><div class="font-monospace mt-2">Username: {{ $voucher->credential->username }}<br>Password: {{ $voucher->credential->password_cipher }}</div><div class="small mt-2">In a router redirect these are submitted automatically.</div></div>@endif
