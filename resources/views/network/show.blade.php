...
<div class="row g-4">
    <div class="col-xl-8">
        <div class="card metric-card mb-4">
            <div class="card-header bg-white d-flex justify-content-between align-items-center"><h2 class="h5 mb-0">Integration Test Center</h2><span class="badge text-bg-{{ $device->status->value === 'online' ? 'success' : ($device->status->value === 'failed' ? 'danger' : 'warning') }}">{{ ucfirst($device->status->value) }}</span></div>
        ...
        <div class="card metric-card">
            <div class="card-header bg-white"><h2 class="h5 mb-0">{{ $provisioning['method'] === 'script' ? 'RouterOS provisioning script' : 'Guided RADIUS configuration' }}</h2></div>
            <div class="card-body">
    ...
    <div class="col-xl-4">
        <div class="card metric-card mb-4"><div class="card-header bg-white"><h2 class="h5 mb-0">Device details</h2></div><div class="card-body">
        ...
        <div class="card metric-card mb-4"><div class="card-header bg-white"><h2 class="h5 mb-0">Capabilities</h2></div><div class="card-body d-flex flex-wrap gap-2">@foreach($device->capabilities ?? [] as $capability)<span class="badge text-bg-light border">{{ str_replace('_', ' ', $capability) }}</span>@endforeach</div></div>
