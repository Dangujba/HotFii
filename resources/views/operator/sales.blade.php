...
<div class="row g-3 mb-4">@foreach([['Online sales','₦'.number_format($totals['online'] / 100, 0),'credit-card','success'],['Voucher activations (pieces)',number_format($totals['voucher']),'ticket-perforated','warning'],['Cash sales','₦'.number_format($totals['cash'] / 100, 0),'cash','primary']] as [$label,$value,$icon,$color])<div class="col-md-4"><div class="card metric-card"><div class="card-body"><div class="text-secondary">{{ $label }}</div><div class="fs-3 fw-bold">{{ $value }}</div><i class="bi bi-{{ $icon }} text-{{ $color }}"></i></div></div></div>@endforeach</div>
...
<div class="card metric-card mb-4"><div class="card-header"><h2 class="h5 mb-0">Record direct cash activation</h2></div><div class="card-body">
...
<x-filter-bar :action="route('sales.index')" :active="$filtered" title="Paid transactions">
    ...
    <div class="col-md-3"><select class="form-select form-select-sm" name="channel"><option value="">All channels</option>@foreach($channels as $channel)<option value="{{ $channel }}" @selected($filters['channel'] === $channel)>{{ $channel === 'cash' ? 'Direct cash' : ucfirst($channel) }}</option>@endforeach</select></div>
...
<div class="card-body p-0"><div class="table-responsive"><table class="table mb-0"><thead><tr><th>Reference</th><th>Router</th><th>Customer</th><th>Plan</th><th>Channel</th><th>Amount</th><th>Status</th></tr></thead><tbody>@forelse($transactions as $transaction)<tr><td><code>{{ $transaction->reference }}</code></td><td>{{ $transaction->networkDevice?->name ?? 'Unattributed' }}</td><td>{{ $transaction->customer?->email ?: 'Walk-in' }}</td><td>{{ $transaction->accessPlan?->name }}</td><td>{{ str_starts_with($transaction->reference, 'HF-VCH-') ? 'Voucher' : ($transaction->channel === 'cash' ? 'Direct cash' : 'Online') }}</td><td>₦{{ number_format($transaction->gross_amount_kobo / 100, 0) }}</td><td><span class="badge text-bg-{{ $transaction->status->value === 'successful' ? 'success' : ($transaction->status->value === 'failed' ? 'danger' : 'warning') }}">{{ ucfirst($transaction->status->value) }}</span></td></tr>@empty<tr><td colspan="7" class="text-center py-5 text-secondary">{{ $filtered ? 'No transactions match these filters.' : 'No transactions yet.' }}</td></tr>@endforelse</tbody></table></div></div></div><div class="mt-3">{{ $transactions->links() }}</div></div>
<div class="col-xl-5"><div class="card metric-card"><div class="card-header"><h2 class="h5 mb-0">Voucher activations</h2></div><div class="list-group list-group-flush">@forelse($voucherSales as $voucher)<div class="list-group-item d-flex justify-content-between"><div><strong>Voucher •••• {{ $voucher->code_last_four }}</strong><div class="small text-secondary">{{ $voucher->batch->accessPlan->name }} · {{ $voucher->activatedNetworkDevice?->name ?? 'Unattributed router' }} · {{ $voucher->sold_at->diffForHumans() }}</div></div><strong>₦{{ number_format($voucher->price_snapshot_kobo / 100, 0) }}</strong></div>@empty<div class="p-5 text-center text-secondary">No activated vouchers.</div>@endforelse</div></div><div class="mt-3">{{ $voucherSales->links() }}</div></div>
</div>
@endsection
