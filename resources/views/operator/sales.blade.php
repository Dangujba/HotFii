...
@section('subheading', 'Online, voucher, and recorded cash access sales')
@section('content')
<div class="row g-3 mb-4">@foreach([['Online sales',$totals['online'],'credit-card','success'],['Voucher activations',$totals['voucher'],'ticket-perforated','warning'],['Cash sales',$totals['cash'],'cash','primary']] as [$label,$amount,$icon,$color])<div class="col-md-4"><div class="card metric-card"><div class="card-body"><div class="text-secondary">{{ $label }}</div><div class="fs-3 fw-bold">₦{{ number_format($amount / 100, 0) }}</div><i class="bi bi-{{ $icon }} text-{{ $color }}"></i></div></div></div>@endforeach</div>
...
<div class="col-xl-5"><div class="card metric-card"><div class="card-header"><h2 class="h5 mb-0">Voucher sales</h2></div><div class="list-group list-group-flush">@forelse($voucherSales as $voucher)<div class="list-group-item d-flex justify-content-between"><div><strong>Voucher •••• {{ $voucher->code_last_four }}</strong><div class="small text-secondary">{{ $voucher->batch->accessPlan->name }} · {{ $voucher->sold_at->diffForHumans() }}</div></div><strong>₦{{ number_format($voucher->price_snapshot_kobo / 100, 0) }}</strong></div>@empty<div class="p-5 text-center text-secondary">No activated vouchers.</div>@endforelse</div></div><div class="mt-3">{{ $voucherSales->links() }}</div></div>
</div>
@endsection