...
<td>@if(in_array($session->status, ['active','disconnect_pending'], true))<form method="POST" action="{{ route('sessions.disconnect', $session) }}">@csrf<button class="btn btn-sm btn-outline-danger" data-confirm="Disconnect this customer now?">Disconnect</button></form>@endif</td>
